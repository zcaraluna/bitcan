      ...
      try {
        ...
        if (template && template.template_html) {
          ...
          const renderData = {
            ...
            ISSUE_DATE: certificate.issued_at ? new Date(certificate.issued_at).toLocaleDateString('es-PY') : new Date().toLocaleDateString('es-PY'),
            issue_date: certificate.issued_at ? new Date(certificate.issued_at).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
