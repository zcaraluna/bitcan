...
// PUT - Actualizar curso (solo campos editables por profesor)
export async function PUT(
  ...
  try {
    ...
    // Actualizar solo los campos permitidos para profesores
    // NO se puede cambiar: identifier, status, is_featured, is_published, sort_order, payment_*, exchange_rate_*
    await query(
      `UPDATE courses SET 
        title = ?, short_description = ?, description = ?, category_id = ?, level = ?,
        ...
        max_students = ?, requires_approval = ?,
        requirements = ?, learning_objectives = ?, thumbnail_url = ?, video_url = ?,
        updated_at = NOW()
        WHERE id = ?`,
      [
        title.trim(),
